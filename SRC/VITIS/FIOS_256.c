...
int main()
{
    ...
    u32 n_arr[s] = {0x8bef, 0xede6, 0x10270, 0x344a, 0x18119, 0x104e8, 0x188b0, 0x1304c, 0x1ba73, 0x1f3c3, 0x161b4, 0x1d114, 0x76d7, 0x2bce, 0xbbf1, 0x1};


    u32 n_prime_0 = 0x1fcf1;

	// a = 0x9c595cfbb4783c24e21a274ac0bc5305030c517928cfdb3b4ad0a51cbe42264e
    u32 a_arr[s] = {0x264e, 0x5f21, 0x2947, 0x1695a, 0xfdb3, 0x1c946, 0x3145, 0xa06, 0xbc53, 0x1a560, 0x8689, 0x1849c, 0x14783, 0xe7dd, 0x7165, 0x1};

	// b = 0x807e96eac45731e81e5e971cdba416d53a47a698cc9c2baa9cdf36bf92dc9938
    u32 b_arr[s] = {0x9938, 0x1c96e, 0x1cdaf, 0x1539b, 0x1c2ba, 0xc664, 0x11e9a, 0x1aa74, 0x1a416, 0x18e6d, 0x197a5, 0x3d03, 0x4573, 0xb756, 0x1fa, 0x1};
    
    
    u32 res[s];

	// expected_res = 0x8462647bc45afc3da53e99bcbace467da14dbb78aa3619409b43c9455ddc4dc3
	u32 expected_res_arr[s] = {0x4dc3, 0xaeee, 0xf251, 0x1368, 0x16194, 0x1c551, 0x136ed, 0xfb42, 0xce46, 0xde5d, 0x14fa6, 0x187b4, 0x45af, 0x123de, 0x1189, 0x1};
    ...
    for(int i = 0; i < s;i++) {
        ...
        Xil_Out32(XPAR_AXI_BRAM_CTRL_0_S_AXI_BASEADDR+4*i, n_arr[i]);

    }

    Xil_Out32(XPAR_AXI_BRAM_CTRL_0_S_AXI_BASEADDR+4*s, n_prime_0);

    for(int i = 0; i < s;i++) {

        Xil_Out32(XPAR_AXI_BRAM_CTRL_0_S_AXI_BASEADDR+4*(s+1+i), a_arr[i]);

    }

    for(int i = 0; i < s;i++) {

        Xil_Out32(XPAR_AXI_BRAM_CTRL_0_S_AXI_BASEADDR+4*(2*s+1+i), b_arr[i]);
    ...
    for(int i = 0; i < 3*s+1; i++){
    	...
    	xil_printf("\nINIT %d : %x", i, Xil_In32(XPAR_AXI_BRAM_CTRL_0_S_AXI_BASEADDR+i*4));
